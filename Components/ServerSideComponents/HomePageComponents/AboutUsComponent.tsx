...
const AboutUsComponent = async () => {
  ...
  return (
    <section className="relative container mx-auto lg:py-6">
      ...
      <div className="max-w-7xl mx-auto px-6  lg:px-8 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
          ...
          <div className="grid ld:grid-cols-2 grid-cols-1 gap-4 md:mb-0 mb-16">
            ...
            {response?.abouts?.counter_number && (
              <div className="absolute lg:top-[80%] top-[33%] lg:left-[8%] left-[18%] text-center">
                ...
                {response?.abouts?.counter_heading && (
                  <p className="text-3xl md:text-4xl text-black text-left custom-spacing">
                    {/* {response?.abouts?.counter_heading} */}
                    Happy <br/>Clients
                  </p>
                )}
          ...
          <div className="flex flex-col gap-3 lg:justify-start justify-center lg:items-start items-center lg:mt-0 mt-8">
            ...
            {response?.heading?.description && (
              <p className="text-2xl font-semibold text-gray-800 lg:text-left text-justify">
                {response?.heading?.description}
              </p>
            )}
            {response?.abouts?.description && (
              <p className="text-gray-800 text-lg lg:text-left text-center py-2">
                {response?.abouts?.description}
              </p>
            )}
            {response?.abouts?.button_text && response?.abouts?.button_link && (
              <Link href={response?.abouts?.button_link} className="">
                <button className="inline-block bg-black text-white md:mt-12 md:py-6 md:px-12 px-6 py-4 rounded-lg hover:bg-gray-800 transition-all ">
